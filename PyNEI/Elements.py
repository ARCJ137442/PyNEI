...
class NARSPerception(): # 现在不需要枚举类
    ...
    @staticmethod
    def new(subject:str, adjective:str):
        "构造感知（与构造函数一致）"
        return NARSPerception(subject, adjective)
    ...
    def __init__(self, subject:str, adjective:str) -> None:
        "构造一个「NARS感知」"
    ...
    def __repr__(self) -> str:
        return "<NARS Perception: {%s} --> [%s] >" % (self.subject, self.adjective)
    
    def __str__(self) -> str:
        "字符串就是其「语句」（一般NAL语法）"
        return "<{%s} --> [%s]>." % (self.subject, self.adjective)

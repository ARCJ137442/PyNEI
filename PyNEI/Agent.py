...
class NARSAgent:
    ...
    def __init__(self, nars_type:NARSType=None, rootPath:str='.', mainGoal:str = None, mainGoal_negative:str = None):  # nars_type: 'opennars' or 'ONA'
        "构造方法"
        # 使用字典记录操作，并在后面重载「__getitem__」方法实现快捷读写操作
        self._operation_container:dict[NARSOperation:int] = dict() # 空字典：获取这个操作「被程序发送了多少次」
        # 使用列表处理感知器
        ...
        nars_type and self.equip_brain(nars_type, rootPath) # 若未输入nars_type，也可以后续再初始化
        # 定义自身的「总目标」

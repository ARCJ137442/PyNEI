...
# 调用的CIN路径还要动态决定（Windows cmd使用反斜杠做路径分割）……
CIN_ROOT_PATH: str = r'..\PyNEI' if IS_ROOT_GAME else r'.\PyNEI'
...
class NARSPlayer(NARSAgent):
    ...
    def __init__(self, nars_type: NARSType = None):
        ...
        self.register_basic_operations(
            *list(NARSPlayer.OPERATION_SET.values()))
        ...
        if self.type == NARSType.ONA:
            if operation.name in NARSPlayer.ONA_OP_LIST:
                operation = NARSPlayer.OPERATION_SET[
                    NARSPlayer.ONA_OP_LIST.index(
                        operation.name) % 9  # 直接「暴力映射」
                ]
...
def getPlayer(prompt: str) -> NARSPlayer | str:
    ...
    try:
        t = NARSType.from_str(type_str) if type_str else NARSType.PYTHON
        return NARSPlayer(t)
    except:
...
def markers_str(markers, row_separator: str = '\n', col_separator: str = '', empty: str = '_'):
...
UPDATE_NARS_EVENT_TIMER = int(
    i if (i := input('Input UPDATE_NARS_EVENT_TIMER: ')) else 200)
timer_update_NARS = int(UPDATE_NARS_EVENT_TIMER)


def set_timers():
    # the activity of NARS
    pygame.time.set_timer(UPDATE_PLAYER_EVENT, timer_update_NARS)
...
max_babble_time: int = int(
    i if (i := input('Input max_babble_time: ')) else 20)
    ...
    if winner:
        for player, nars in players.items():
            if player == winner:
                continue
            # 清除
            playerType = players[player].type.value if (
                isNARS := isinstance(nars, NARSPlayer)) else players[player]
            print(
                f'Deleted player {player}({playerType}) with n_win={stat_win.get(player, 0)}')
            if isNARS:
                delete_nars(player)
            else:
                players[player] = None
        print(
            f'Picked player {winner}({players[winner]}) with n_win={max_win}')
    else:  # 否则继续训练
        pretrain(count)
    # 清除胜利统计
    clearStat(stat_win)
...
def handle_operation(nars: NARSPlayer, player: str) -> bool:
    ...
    active_operation_name_list = list(nars.active_operation_names)
    for operation_name in active_operation_name_list:  # 使用list防止运行时改变
